 ...
 */
public class DiccionarioRespaldo {
...
public String traducirCaracterACaracter(String textoATraducir, int indiceIdioma) {
        ...
        for (String linea : lineas) {
            Arrays.stream(extraerPalabras(linea))
                    .forEach(palabra -> {
                        StringBuilder caracterTraducido = new StringBuilder();
                        ...
                        if (esEspanol) {
                            ...
                            palabra.chars().forEach(c -> {
                                char caracter = (char) c;
                                if (contieneNumeros(String.valueOf(caracter))) {
                                    caracterTraducido.append(CARACTER_NUMERO);
                                }
                                if (Character.isUpperCase(caracter)) {
                                    caracterTraducido.append(CARACTER_MAYUSCULAS);
                            ...
                            // Traducción a español
                            boolean isNumber = false;  // Estado para identificar si el contexto actual es numérico
                            int bandera = 0;
                            // 0 para letras
                            //1 numeros
                            ...
                            for (int i = 0; i < palabra.length(); i++) {
                                ...
                                if (caracter.equals(String.valueOf(CARACTER_NUMERO))) {
                                    bandera = 1;
                                }
                                if (caracter.equals(String.valueOf(CARACTER_LETRA))) {
                                    bandera = 0;
                                }
...
//                                    char traducido;
//
//                                    i++;  // Saltar al siguiente caracter que debería ser traducido a mayúscula
//                                    caracter = String.valueOf(palabra.charAt(i));
//                                    numero.append(caracter);
//                                    // Manejar mayúsculas y traducción estándar
//
//                                    String numeroString = String.valueOf(numero);
//
//                                    traducido = String.valueOf(obtenerCaracterEnEspaniol(numeroString)).charAt(0);
                                     
                                    i++;
                                    
                                    // iterar sobre todos los números y parar cuando haya un caracter de símbolo o letra, o espacio
                                    while (!caracter.equals(String.valueOf(CARACTER_LETRA)) || !caracter.equals(String.valueOf(CARACTER_SIMBOLO)) || !caracter.equals(String.valueOf(CARACTER_LETRA)) || !caracter.equals(String.valueOf(" ")) || i < palabra.length()) {
                                        System.out.println(i);
                                        
                                        ch = palabra.charAt(i);
                                        caracter = String.valueOf(ch);
                                        
                                        numeros.append(caracter);
                                        i++;
                                    }
                                    
                                    String numerosString = String.valueOf(numeros);
                                   
                                    // construir la subpalabra con los números
                                    String[] numerosBrailleSeparados = separarNumerosBraille(agregarCaracterNumerico(numerosString));
                                    for (String numero : numerosBrailleSeparados) {
                                        caracterTraducido.append(obtenerCaracterEnEspaniol(numero));
                                    }
                                                                       

//                                    while (!esCaracterLetra) {
//
//                                        if (caracter.equals("⠰")) {
//                                            break; // Salir del bucle si encuentra '⠰'
//                                        }
//                                        numero.append(caracter);
//                                        i++;  // Saltar al siguiente caracter
//                                    }
                                } else if (bandera == 0) {
// Manejar mayúsculas y traducción estándar
                                    boolean nextIsUppercase = caracter.equals(CARACTER_MAYUSCULAS);
                                    if (nextIsUppercase) {
                                        i++;  // Saltar al siguiente caracter que debería ser traducido a mayúscula
                                        caracter = String.valueOf(palabra.charAt(i));
                                    }
                                    char traducido = String.valueOf(obtenerCaracterEnEspaniol(caracter)).charAt(0);
                                    if (nextIsUppercase) {
                                        traducido = Character.toUpperCase(traducido);
                                    }
                                    caracterTraducido.append(traducido);
